...
- **★ 7.5 篇级批量生成面板 v1.1（用户验收插队拍板，纯前端编排后端零改动，vite build 通过）**：
  ...
  - **三条复用链路核查定论**：① `chapter.py` 生成端点 `article_id`+空 `chapter_id` → 按"本篇最大序号+1"
    自动接续（前端 `chapter_no` 仅 schema 占位）；② confirmed 计划由 `layer_chapter_plan` 按
    chapter_no 对行 **P_CRITICAL 注入**（`builder.py:213`）——拍板的计划生成时自动生效；
    ③ `discussionAppend` 可往指定章线程写消息（`discussion_messages` 落 SQLite 永久保存）。
  - **实现**（`PlanView.vue` 批量区）：拍板后页头绿色「一键生成本篇（N 章）」→ 确认弹窗（起止行/
    时长/勿关页）→ 逐章循环 SSE（`prompt_hint` 拼该行计划兜底；`enable_thinking=true`、
    `temperature=0.75` 与手动弹窗同源防复读）→ 每章完成 `discussionAppend` **留两条消息**
    （user 生成指令 + ai 完成回执——用户拍板的留痕形式）→ `loadStructure` 侧栏树实时刷新。
  - 面板：进度表（待生成/生成中/完成·字数/失败·悬停原因/已有跳过）+ 当前章流式正文滚动区 +
    暂停（本章跑完停）/终止（立即断流，未落库章不保存）/单行重试/重试全部失败；
    **续跑对齐** = startBatch 先查篇内已有章数跳过前 N 行（关页重开后从断点续跑）；
    批量期间切换小说/篇被拦截。局限：前端编排关页即停（已生成章不受影响），"关页也跑"留待 7.6 后台化。
  - 顺带：「章节列表」补侧栏入口（`chapters` 路由早已存在但零入口——**零入口病第三例**）。
